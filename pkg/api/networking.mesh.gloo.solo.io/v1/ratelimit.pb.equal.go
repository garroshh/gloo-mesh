...
func (m *RatelimitConfig) Equal(that interface{}) bool {
	...
	if h, ok := interface{}(m.GetDescriptors()).(equality.Equalizer); ok {
		if !h.Equal(target.GetDescriptors()) {
			return false
		}
	} else {
		if !proto.Equal(m.GetDescriptors(), target.GetDescriptors()) {
			return false
		}
	}

	if h, ok := interface{}(m.GetSetDescriptors()).(equality.Equalizer); ok {
		if !h.Equal(target.GetSetDescriptors()) {
			return false
		}
	} else {
